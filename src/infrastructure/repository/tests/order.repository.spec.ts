...
describe("Order repository test", () => {
    ...
    it("should throw an error when customer orders aren't found", async() => {
        const orderRepository = new OrderRepository();

        const customer = new Customer("123", "Customer 1");

        expect(async () => {
            await orderRepository.findAll(customer);
        }).rejects.toThrow("Customer 1 hasn't created any orders yet!");
    });

    it("should find all orders of the customer", async() => {
        const customerRepository = new CustomerRepository();
        const customer = new Customer("123", "Customer 1");
        customer.address = new Address("Street 1", 1, "Zipecode 1", "City 1");
        await customerRepository.create(customer);
    ...
    it("should find all orders of the customer", async() => {
        ...
        const orders = await orderRepository.findAll(customer);
